...
def test_database_api_returns_building_10_with_22_units():
    """Test that database API correctly returns building 10 with all 22 apartments"""
    units = database_api.get_all_units()
    building_10_units = [u for u in units if u['buildingNumber'] == 'عمارة 10']
    
    assert len(building_10_units) == 22, f"API should return 22 units for building 10, but returned {len(building_10_units)}"
...
def test_building_10_metadata_updated():
    ...
    assert total_units == 22, f"Building 10 total_units should be 22, but is {total_units}"
