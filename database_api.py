    ...
    try:
        cursor.execute('''
            INSERT INTO residents (
                name, national_id, phone, email, unit_id, move_in_date, status
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            resident_data.get('name'),
            resident_data.get('national_id'),
            resident_data.get('phone'),
            resident_data.get('email'),
            resident_data.get('unit_id'),
            resident_data.get('move_in_date'),
            resident_data.get('status', 'نشط')
        ))
        ...
        if resident_data.get('unit_id'):
            cursor.execute('''
                UPDATE units SET status = 'مشغول' WHERE id = ?
            ''', (resident_data.get('unit_id'),))
            conn.commit()
...
def update_resident(resident_id, resident_data):
    ...
    try:
        # الحصول على unit_id القديمة
        cursor.execute('SELECT unit_id FROM residents WHERE id = ?', (resident_id,))
        result = cursor.fetchone()
        if not result:
            conn.close()
            return {
                'success': False,
                'error': 'الساكن غير موجود',
                'message': 'فشل في تحديث الساكن'
            }
        
        old_unit_id = result['unit_id']
        new_unit_id = resident_data.get('unit_id')
        
        # تحديث معلومات الساكن
        cursor.execute('''
            UPDATE residents 
            SET name = ?, phone = ?, email = ?, unit_id = ?, status = ?
            WHERE id = ?
        ''', (
            resident_data.get('name'),
            resident_data.get('phone'),
        ...
        # إذا تغيرت الوحدة، تحديث حالة الوحدات
        if old_unit_id != new_unit_id:
            if old_unit_id:
                cursor.execute('UPDATE units SET status = "شاغر" WHERE id = ?', (old_unit_id,))
            if new_unit_id:
                cursor.execute('UPDATE units SET status = "مشغول" WHERE id = ?', (new_unit_id,))
...
def checkout_resident(resident_id, checkout_data=None):
    """إخراج/سحب ساكن (move out)"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    try:
        # الحصول على معلومات الساكن
        cursor.execute('SELECT unit_id FROM residents WHERE id = ?', (resident_id,))
        result = cursor.fetchone()
        if not result:
            conn.close()
            return {'success': False, 'error': 'الساكن غير موجود', 'message': 'فشل في إخراج الساكن'}
        
        unit_id = result['unit_id']
        
        # تحديث حالة الساكن إلى "مغادر"
        cursor.execute('UPDATE residents SET status = ?, unit_id = NULL WHERE id = ?', ('مغادر', resident_id))
        
        # تحرير الوحدة
        if unit_id:
            cursor.execute('UPDATE units SET status = ? WHERE id = ?', ('شاغر', unit_id))
        
        # تعطيل الملصقات المرتبطة بالساكن
        cursor.execute('UPDATE vehicle_stickers SET status = ? WHERE resident_id = ?', ('ملغي', resident_id))
        
        conn.commit()
        conn.close()
        return {'success': True, 'message': 'تم إخراج الساكن بنجاح'}
    
    except Exception as e:
        conn.close()
        return {'success': False, 'error': str(e), 'message': 'فشل في إخراج الساكن'}
...
def delete_resident(resident_id):
    ...
    try:
        ...
        if not result:
            conn.close()
            return {'success': False, 'error': 'الساكن غير موجود', 'message': 'فشل في حذف الساكن'}
        ...
        conn.commit()
        conn.close()
        return {'success': True, 'message': 'تم حذف الساكن بنجاح'}
    
    except Exception as e:
        conn.close()
        return {'success': False, 'error': str(e), 'message': 'فشل في حذف الساكن'}


def get_available_units(building_id=None):
    ...
    if building_id:
        cursor.execute('''
            SELECT u.id, u.unit_number as unitNumber, u.unit_type as unitType,
                   u.status, b.building_number as buildingNumber
            FROM units u JOIN buildings b ON u.building_id = b.id
            WHERE u.status = ? AND u.building_id = ?
